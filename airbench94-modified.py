...
hyp = {
    'opt': {
        'train_epochs': 8.0,
        'batch_size': 1536,
        'lr': 17.25,                 # learning rate per 1024 examples
        'momentum': 0.85,
...
class CifarLoader:
    ...
    def __init__(self, path, train=True, batch_size=500, aug=None, drop_last=None, shuffle=None, gpu=0):
        ...
        self.images = data['images']    # uint8 [N,H,W,C]
        self.labels = data['labels'].long().to(device)
        self.classes = data['classes']
    
        # Convert once → GPU-friendly format
        self.images = (
        ...
        self.aug = aug or {}
        for k in self.aug:
            assert k in ['flip', 'translate'], f"Unrecognized aug: {k}"
        ...
        # ----------------------------------------------------
        #  PRECOMPUTE ALL STATIC TRANSFORMS (INSTANT SPEEDUP)
        # ----------------------------------------------------
        with torch.no_grad():
            base = self.normalize(self.images)
            self.proc_images['norm'] = base
    
            if self.aug.get('flip', False):
                # flip all images once
                self.proc_images['flip'] = base.flip(-1)
    
            pad = self.aug.get('translate', 0)
            if pad > 0:
                # reflect-pad once and reuse every epoch
                self.proc_images['pad'] = F.pad(base, (pad, pad, pad, pad), mode='reflect')
    
        self.epoch = 0


    def __iter__(self):
        flip = self.aug.get('flip', False)
        pad  = self.aug.get('translate', 0)
    
        # -------------------------
        # Select source image tensor
        # -------------------------
        if pad > 0:
            # random crop every epoch
        ...
        # Flip whole batch every other epoch
        if flip and (self.epoch % 2 == 1):
        ...
        # -------------------------
        N = images.shape[0]
        num_batches = N // self.batch_size if self.drop_last else ceil(N / self.batch_size)
    
        indices = (torch.randperm if self.shuffle else torch.arange)(N, device=images.device)
...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        ...
        self.activ = nn.GELU()
        # residual connection
        self.downsample = nn.Conv2d(channels_in, channels_out, kernel_size=1, stride=1, bias=False)
