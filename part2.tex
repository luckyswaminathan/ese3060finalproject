...
We start from the provided \texttt{train\_gpt.py} configuration. The modified MLP is implemented in
\texttt{train\_gpt\_swiglu.py} (\url{https://github.com/luckyswaminathan/ese3060finalproject/blob/main/train_gpt_swiglu.py});
the baseline remains in \texttt{train\_gpt.py}. All hyperparameters are kept fixed: same model size
(\texttt{GPTConfig(n\_layer=12, n\_head=6, n\_embd=768)}), global batch size, sequence length $T=1024$,
optimizers (AdamW for the head and Muon for the transformer blocks), learning rate schedule, and
number of iterations ($1500$ steps).
