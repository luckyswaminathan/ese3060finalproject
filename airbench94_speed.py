...
hyp = {
    'opt': {
        # fewer epochs → ~20% less total compute
        'train_epochs': 8.0,         # baseline: 9.9

        # larger batch → fewer steps per epoch
        'batch_size': 1536,          # baseline: 1024

        # scale LR linearly with batch size
        # 11.5 * (1536 / 1024) = 17.25
        'lr': 17.25,
...
class CifarLoader:
    ...
    def __init__(self, path, train=True, batch_size=500, aug=None, drop_last=None, shuffle=None, gpu=0):
        ...
        self.color_jitter = T.ColorJitter(brightness=0.15, contrast=0.15)
        self.proc_images = {} # Saved results of image processing to be done on the first epoch
        self.epoch = 0

        self.aug = aug or {}
        for k in self.aug.keys():
            assert k in ['flip', 'translate', 'color_jitter'], 'Unrecognized key: %s' % k
    ...
    def __iter__(self):
        ...
        if self.epoch == 0:
            # normalize first
            images = self.normalize(self.images)

            # light color jitter once per epoch
            if self.aug.get('color_jitter', False):
                # safer to apply in float then go back to half
                images = self.color_jitter(images.float()).half()

            # save the normalized (and maybe jittered) base images
